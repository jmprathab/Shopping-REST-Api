...
public class AccountsResource {
	...
	private boolean validateUserData(Users users) {
		return !Validators.isStringEmpty(users.getName()) && !Validators.isStringEmpty(users.getMobile())
				&& !Validators.isStringEmpty(users.getPassword());
	}

	@ApiOperation(value = "Login to the existing User account", notes = "Login requires mobile and password")
	@ApiResponses(value = {
	...
	public Response loginTheUser(
		...
		if (Validators.isStringEmpty(inputUsers.getMobile()) || Validators.isStringEmpty(inputUsers.getPassword())) {
		...
		if (isUserDataValid) {
			...
			if (fetchedUsers == null || !BCrypt.checkpw(inputUsers.getPassword(), fetchedUsers.getPassword())) {
			...
			System.out.println(inputUsers.getName() + " " + inputUsers.getMobile() + " " + inputUsers.getPassword());

			String jwt = createAndGetJWT(claims);

			if (jwt != null) {
				return Response.ok(inputUsers, MediaType.APPLICATION_JSON_TYPE)
						.header(HttpConstants.HTTP_HEADER_TOKEN, jwt).build();
			}
			return Response.status(Status.BAD_REQUEST).build();
		}
		return Response.status(Status.UNAUTHORIZED).build();
	...
	@ApiOperation(value = "Reset Account Password", notes = "Forgot Password requires email")
	@ApiResponses(value = {
	...
	public Response forgotPassword(
			@ApiParam(value = "Email id of User", required = true) @FormParam(HTTP_PARAM_EMAIL) String email) {
	...
	@ApiOperation(value = "Delete User's account", notes = "Delete account requires mobile and password")
	@ApiResponses(value = {
	...
	@Path("/delete")
	@Consumes({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	public Response deleteAccount(@ApiParam(value = "Should contain mobile, password", required = true) Users users) {

		boolean isUserDataValid = true;
		if (Validators.isStringEmpty(users.getMobile()) || Validators.isStringEmpty(users.getPassword())) {
		...
		if (isUserDataValid) {
			...
			if (fetchedUsers == null || !BCrypt.checkpw(users.getPassword(), fetchedUsers.getPassword())) {
